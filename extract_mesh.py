...
def get_udf_normals_grid_slow(func, func_grad, N=56, max_batch=int(2 ** 20)):
    ...
    while head < num_samples:
        # xyz coords
        sample_subset = samples[head: min(head + max_batch, num_samples), 0:3].clone().cpu()
        # Create input
        xyz = sample_subset
    ...
    while head < num_samples:
        ...
        # xyz coords
        sample_subset = samples[sample_subset_mask, 0:3].clone().cpu()
...
def get_udf_normals_grid_fast(func, func_grad, samples, indices, N=56, max_batch=int(2 ** 18)):
    ...
    while head < num_samples:
        # xyz coords
        sample_subset = samples[indices[head: min(head + max_batch, num_samples)], 0:3].clone().cpu()
        sample_subset.requires_grad = True
        # Create input
        xyz = sample_subset
...
def get_mesh_udf_fast(func, func_grad, samples=None, indices=None, N_MC=128,
    ...
    verts_torch = torch.from_numpy(verts).float().cpu()
    with torch.no_grad():
        xyz = verts_torch
    ...
    if not gradient:
        return torch.tensor(filtered_mesh.vertices).float().cpu(), torch.tensor(
            filtered_mesh.faces).long().cpu(), filtered_mesh
    else:
        ...
        normals = torch.tensor(normals).float().cpu()
        verts = torch.tensor(filtered_mesh.vertices).float().cpu()
        ...
        if border_gradients:
            ...
            edge_border = torch.tensor(edge_border).float().cpu()
            ...
            border_verts = torch.tensor(filtered_mesh.vertices[u_border]).float().cpu()
        ...
        return new_verts, torch.tensor(filtered_mesh.faces).long().cpu(), filtered_mesh, samples, next_indices
